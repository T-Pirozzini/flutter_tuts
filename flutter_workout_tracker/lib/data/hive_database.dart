...
class HiveDatabase {
  ...
  List<Workout> readFromDatabase() {
    ...
    for (int i = 0; i < workoutNames.length; i++) {
      ...
      for (int j = 0; j < exerciseDetails[i].length; j++) {
        // so add each exercise into the list
  ...
  // return completion status of a given dat yyyymmdd
...
List<List<List<String>>> convertObjectToExerciseList(List<Workout> workouts) {
  List<List<List<String>>> exerciseList = [
    /*
      [
        ...
        Lower Body
        [ [squats, 10kg, 10reps, 3sets], [legraise, 20kg, 10reps, 3sets] ],
      ]
    */
  ];
  ...
  for (int i = 0; i < workouts.length; i++) {
    ...
    List<Exercise> exercisesInWorkut = workouts[i].exercises;
    ...
    // got through each exercise in exerciseList
    for (int j = 0; j < exercisesInWorkut.length; j++) {
      List<String> individualExercise = [
        // [biceps, 10kg, 10reps, 3sets]
      ];
      individualExercise.addAll(
        [
          exercisesInWorkut[j].name,
          exercisesInWorkut[j].weight,
          exercisesInWorkut[j].reps,
          exercisesInWorkut[j].sets,
          exercisesInWorkut[j].isCompleted.toString(),
        ],
      );
